...
local robo = Proto("myrobo1","Falcon Protocol DCAP")

local ef_too_short = ProtoExpert.new("myrobo.too_short.expert", "DNS message too short",
                                     expert.group.MALFORMED, expert.severity.ERROR)

local pf_command            = ProtoField.uint16("myrobo1.command", "Command")
local pf_address            = ProtoField.uint16("myrobo1.address", "Address")
local pf_data               = ProtoField.int32("myrobo1.data", "Data")
...
function robo.dissector(tvbuf,pktinfo,root)
    ...
    pktinfo.cols.protocol:set("FALCON_DCAP") -- set the protocol column to show our protocol name
    ...
    for key, value in pairs(items) do
        ...
        if command_type == 1 then
            ...
            if key == 1 then -- command field
                if value == '1' then
                    tree:add("Command: " .. value .. " [Start Recording]")
                elseif value == '0' then
                    tree:add("Command: " .. value .. " [Stop Recording]")
                else
                    tree:add("Command: " .. value .. " [Invalid]")
                end

            elseif key == 2 then -- addresses field

                local address_tree = tree:add("Addresses")
                -- value.len() - 1
                value = string.sub(value, 2, string.len(value) - 1)

                local addrs = split(value, ',')

                for k, v in pairs(addrs) do
                    address_tree:add("Element " .. k .. ": " .. v)
                end

            elseif key == 3 then -- rate field

                tree:add("Rate: " .. value)
            
            elseif key == 4 then -- file name field

                tree:add("Filename: " .. value)
